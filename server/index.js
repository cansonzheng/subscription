...
const JWT_SECRET = process.env.JWT_SECRET || 'v2ray-subscription-jwt-secret-key-2026';
...
router.post('/api/login', async (ctx) => {
  ...
  if (inputPassword === storedPassword) {
    const token = generateToken({ loggedIn: true, iat: Math.floor(Date.now() / 1000) });
...
router.put('/api/groups/:id', requireAuth, async (ctx) => {
  ...
  // 过滤空行
  const filteredProxies = (Array.isArray(proxies) ? proxies : []).filter(p => p.trim());
